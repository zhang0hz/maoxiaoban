...
## V1.1 Formal Asset Library

- `ASSET_NAMING.md`: action ID, frame, manifest, and cleanup rules.
- `formal-assets.json`: runtime-approved action list and deprecated-material policy.
- `action-manifest.json`: authoritative action metadata; formal runtime actions use `status: "formal"`.

Runtime code should reference only action IDs present in `frames/` and recorded in the formal asset list.
