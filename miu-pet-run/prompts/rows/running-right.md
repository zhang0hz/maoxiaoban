Create a single horizontal sprite strip for the Codex app digital pet `miu` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Blue bicolor ragdoll cat, soft white body, cool gray-blue ears and tail, blue mask on upper face, bright blue eyes, tiny pink nose, fluffy cheeks, gentle and cute temperament, calm companion energy..
