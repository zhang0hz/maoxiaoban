...
extension MiuRunner {
    ...
    func makeSettingsWindow() -> NSWindow {
        ...
        tabView.addTabViewItem(tabItem(title: "分类", view: makeClassificationSettingsView()))
    ...
    func makeClassificationSettingsView() -> NSView {
        let stack = settingsStack()
        let title = NSTextField(labelWithString: "当前应用分类")
        title.font = NSFont.boldSystemFont(ofSize: 14)
        stack.addArrangedSubview(title)
    ...
    func makeDebugSettingsView() -> NSView {
        ...
        let refresh = NSButton(title: "刷新调试信息", target: self, action: #selector(refreshDebugPanel))
